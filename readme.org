...
A [[https://devenv.sh/][devenv]] shell is included for local builds.

1. Enter the shell with ~nix develop --no-pure-eval~ or ~devenv shell~.
2. For automatic shell activation with direnv, run ~direnv allow~.  The included ~.envrc~ passes ~--accept-flake-config~ so the devenv Cachix substituter is accepted automatically.
3. Initialise the local workspace with ~miryoku-zmk-init~.
4. Build firmware with ~miryoku-zmk-build~.
...
The shell sets ~ZMK_CONFIG~, configures the GNU Arm Embedded toolchain, creates a local workspace under [[.local]], and reuses the repository's outboard metadata for local builds of out-of-tree boards and shields.

The first run bootstraps a Python virtual environment under [[.local/venv]] and installs the ZMK and Zephyr Python requirements into it.
